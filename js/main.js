(() => {
	...
	if (packageGrid) {
		...
		const specialistPricing = {
			paint: { hatch: { amount: 420, label: 'Hatchback' }, sedan: { amount: 480, label: 'Sedan' }, seven: { amount: 579, label: '5-Seater SUV' }, '4wd': { amount: 580, label: 'UTE' } },
			ceramic: { hatch: { amount: 779, label: 'Hatchback' }, sedan: { amount: 880, label: 'Sedan' }, seven: { amount: 1080, label: '5-Seater SUV' }, '4wd': { amount: 1080, label: 'UTE' } }
		};
		const updateSpecialistPricing = () => {
			const priceSections = [
				{ section: paintSection, pricing: specialistPricing.paint },
				{ section: ceramicSection, pricing: specialistPricing.ceramic }
			];
			priceSections.forEach(({ section, pricing }) => {
				...
				const selected = pricing[vehicle];
				if (amount && unit && selected) {
					amount.textContent = `$${selected.amount}`;
					unit.textContent = `AUD / ${selected.label}`;
				}
			});
		};
		const updateCeramicVehicle = () => { if (ceramicVehicle) ceramicVehicle.textContent = `Available for ${vehicleNames[vehicle]}`; updateSpecialistPricing(); };
